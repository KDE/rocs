...
bool FileFormatDialog::exportFile(GraphDocumentPtr document) const
{
    ...
    const QUrl file = exportDialog->selectedFiles().count() ? QUrl::fromLocalFile(exportDialog->selectedFiles().first()) : QUrl();
    ...
    if (QFile::exists(file.toLocalFile())) {
        if (KMessageBox::warningContinueCancel(qobject_cast<QWidget *>(parent()),
                                               i18n("<p>The file <br /><strong>'%1'</strong><br /> already exists; if you "
    ...
    if (ext.isEmpty() || !filePlugin) {
        KMessageBox::error(qobject_cast<QWidget *>(parent()),
                           i18n("<p>Cannot resolve suffix of file <strong>'%1'</strong> to an available file backend. "
                                "Aborting export.</p>",
                                file.toDisplayString()));
    ...
    if (filePlugin->hasError()) {
        KMessageBox::error(qobject_cast<QWidget *>(parent()),
                           i18n("<p>Error occurred when writing file: <strong>'%1'</strong></p>", filePlugin->errorString()));
...
GraphDocumentPtr FileFormatDialog::importFile()
{
    ...
    QPointer<QFileDialog> dialog = new QFileDialog(qobject_cast<QWidget *>(parent()), i18nc("@title:window", "Import Graph File into Project"));
    if (!dialog->exec()) {
